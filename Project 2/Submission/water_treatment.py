...
def reduce_dimensions(X, X_not_scaled):
    '''
    ...
    X_r = reducer.fit_transform(X)
    yield 'RandProj', X_r[:, 0], X_r[:, 1]

    from sklearn.decomposition import LatentDirichletAllocation
    reducer = LatentDirichletAllocation(n_topics=2, learning_method='batch', n_jobs=-1)
    X_r = reducer.fit_transform(X_not_scaled)
...
def evaluate_learners(X):
    ...
    learner = GaussianMixture(n_components=2).fit(X)
    y = learner.predict(X)
    yield 'Expectation Maximization clusters', y

    learner = MiniBatchKMeans(n_clusters=2)
    y = learner.fit_predict(X)
    yield 'K Means clusters', y

    learner = GaussianMixture(n_components=3).fit(X)
    y = learner.predict(X)
    yield 'Expectation Maximization clusters', y
    ...
    learner = GaussianMixture(n_components=4).fit(X)
    y = learner.predict(X)
    yield 'Expectation Maximization clusters', y

    learner = MiniBatchKMeans(n_clusters=4)
    y = learner.fit_predict(X)
    yield 'K Means clusters', y
...
if __name__ == '__main__':
    ...
    X, X_not_scaled = get_features(frame)

    # Run multiple dimensionality reduction algorithms on the data
    print("Reducing dimensionality")
    Xs = list(reduce_dimensions(X,X_not_scaled))
