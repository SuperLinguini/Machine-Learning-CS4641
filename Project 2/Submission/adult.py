...
def kmeans(train_data, test_data, output_str, i=0):
    ...
    fig = ax.get_figure()
    fig.savefig(os.path.join('Images','adult_kmeans_{}{}.png'.format(output_str, i)))
...
def expectation_maximization(train_data, test_data, output_str, i=0):
    ...
    fig = ax.get_figure()
    fig.savefig(os.path.join('Images','adult_expectation_maximization_{}{}.png'.format(output_str, i)))
...
def neural_network(X_train, y_train, X_test, y_test, output_str, i):
    ...
    classifier = tf.contrib.learn.DNNClassifier(feature_columns=feature_columns,
                                                hidden_units=[10, 10],
                                                # dropout=0.5,
                                                model_dir="/tmp/adult_nn_{}{}".format(output_str, i),
...
def kmeans_as_feature(i=0):
    ...
    for n in n_clusters:
        ...
        if type(X_train) is np.ndarray:
            X_train_feature = pd.DataFrame(X_train.copy())
        else:
            X_train_feature = X_train.copy()
        X_train_feature.index = range(X_train_feature.shape[0])
        X_train_feature['Clusters'] = pd.Series(k_means.predict(X_train_feature))

        if type(X_test) is np.ndarray:
            X_test_feature = pd.DataFrame(X_test.copy())
        else:
            X_test_feature = X_test.copy()
        X_test_feature.index = range(X_test_feature.shape[0])
        X_test_feature['Clusters'] = pd.Series(k_means.predict(X_test_feature))
...
def expectation_maximization_as_feature(i=0):
    ...
    for n in n_clusters:
        ...
        if type(X_train) is np.ndarray:
            ...
            X_train_feature = X_train.copy()
        X_train_feature.index = range(X_train_feature.shape[0])
        X_train_feature['Clusters'] = pd.Series(gm.predict(X_train_feature))
        ...
        if type(X_test) is np.ndarray:
            ...
            X_test_feature = X_test.copy()
        X_test_feature.index = range(X_test_feature.shape[0])
        X_test_feature['Clusters'] = pd.Series(gm.predict(X_test_feature))

        neural_network(X_train_feature, y_train, X_test_feature, y_test, 'EMFeatures{}_'.format(i), n)

def double_data(i):
    X_train_feature = X_train.append(X_train)
    y_train_feature = y_train.append(y_train)
    ...
    neural_network(X_train_feature, y_train_feature, X_test_feature, y_test, 'Double{}_'.format(i), i)
